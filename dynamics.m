classdef dynamics < handle
    ...
    methods
        function self = dynamics(core)
            ...
            self.sensor_names = core.param.sensor_names;
...
%             end
        ...
        function simulate(self)
            ...
            for i = 1:length(r)-1
                ...
                % Observe
                x_hat = zeros(size(sensor_readings));
                ...
                for j = 1:length(self.observers)
                    ...
                    [x_hat(indexes),d_hat(j)] = self.observers(j).observe(sensor_readings,t(i));
                end
                    
                % Implimennt controller
                self.u = self.controller_architecture(self.controllers,x_hat,r(:,i),d_hat,self.param);
